  ...
  for (const ci of active) {
    ...
    try {
      ...
      for (const product of products) {
        ...
        // Seed or refresh inventory quantity from Shopify for this product
        ...
        const channelInventoryItemId =
          firstVariant?.inventory_item_id != null ? String(firstVariant.inventory_item_id) : raw.variants?.[0]?.inventory_item_id != null ? String(raw.variants[0].inventory_item_id) : null;
        if (slug === "shopify" && channelProductId) {
          await channelProductsColl.updateOne(
            { product_id: productId, channel_name: "shopify" },
            {
              $set: {
